...
class TestSendRpc:
    ...
    async def test_no_response(self, connection, data, expected_payload):
        ...
        await rpc.connect()
        await rpc.send_rpc(
            destination=TEST_DESTINATION, data=data, await_response=False
        )
        rpc.channel.basic_publish.assert_called_once_with(
            exchange_name="",
            routing_key=TEST_DESTINATION,
            properties={},
            payload=expected_payload,
        )

    @pytest.mark.parametrize(
        "data, sent_payload, received_payload, expected_result",
        [
            ...
            (
                "request data",
                b"request data",
                b"Test. \xd0\xa2\xd0\xb5\xd1\x81\xd1\x82",
                "Test. Тест",
            ),
            (b"raw request", b"raw request", b"Raw message \xff", b"Raw message \xff"),
        ],
    )
    async def test_await_response(
        self, connection, data, sent_payload, received_payload, expected_result
    ):
        """Check that data returned by await_response is parsed and returned correctly."""
        rpc = cabbage.AsyncAmqpRpc(connection=connection)
        await rpc.connect()
        with patch(
            "cabbage.amqp.AsyncAmqpRpc._await_response",
            return_value=received_payload,
            autospec=True,
        ), patch("cabbage.amqp.uuid.uuid4", return_value=RESPONSE_CORR_ID):
            result = await rpc.send_rpc(
                destination=TEST_DESTINATION, data=data, await_response=True
            )
...
class TestAwaitResponse:
    ...
    async def test_ok(self, rpc):
        ...
        task = asyncio.ensure_future(
            rpc._await_response(correlation_id=RESPONSE_CORR_ID, timeout=10.0)
        )
...
class TestOnResponse:
    ...
    @pytest.mark.parametrize(
        "body", [b"", b"Test message body. \xd0\xa2\xd0\xb5\xd1\x81\xd1\x82"]
    )
